...
def scrape_esma_page(url):
    ...
    articles = table.find_all("tr")  # Adjust class as per ESMA website structure
    scraped_data = []
    ...
    for article in articles:
        try:
            ...
            document_data = {
                ...
                "source_website": "www.esma.europa.eu/news-publications/speeches",
                "retrieved_at": datetime.now().isoformat(),
                "id": generate_unique_id(link),
                "type": "speech",  # Default category, can be modified dynamically
                "title": title,
                "url": link,
...
def scrape_detailed_article(url):
    ...
    detailed_data['full_html'] = page_source
...
def scrape_all_esma_pages():
    ...
    with open("esma_scraped_speech_data.json", "w", encoding="utf-8") as f:
